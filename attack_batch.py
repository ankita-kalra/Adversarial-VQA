...
class Attacker:
	...
	if not val:
	    ...
	    if not val:
            ...
            if self.targetted_const == -1:
                ############################################## Works only with batch_size = 1 for now. Modify for larger sizes later
                #if(loss2.data.cpu().numpy()[0] < 26000 and (ans_index.numpy()[0] != target_idx.numpy()[0])): ###Check   #Condition that noise levels are thresholded and targetted attack is successful

                    #print(self.VQA_model.index_to_answer[target_idx.numpy()[0]], self.VQA_model.index_to_answer[ans_index.numpy()[0]])

                    #img_np1 = self.unorm(img_.data).cpu().numpy()
                    #img_cv = np.transpose(img_np1,(0,2,3,1))
                    #img_cv = cv2.convertScaleAbs(img_cv.reshape(img_cv.shape[0],448,448,3)*255)
                    #img_cv = cv2.cvtColor(img_cv, cv2.COLOR_BGR2RGB)
                    #cv2.imshow('purturbed', img_cv)
                    #cv2.waitKey(0)
                    #cv2.imshow('purturbed', img_cv)
