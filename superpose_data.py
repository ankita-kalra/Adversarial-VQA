...
class VQA(data.Dataset):
    ...
    def __init__(self, questions_path1, questions_path2, answers_path1, answers_path2, image_path, fdict_path, answerable_only=False):
    ...
    def __getitem__(self, item):
        ...
        q1, q_length1 = self.questions1[item]
	q2, q_length2 = self.questions2[item]
        a1 = self.answers1[item]
	a2 = self.answers2[item]
        image_id1 = self.coco_ids1[item]
	image_id2 = self.coco_ids2[item]
        
        #Use this to load resnet feats
        #v = self._load_image(image_id)

        #Use this to lead RGB feats
        v1 = self._load_rgb_image(image_id1, self.transform)
	v2 = self._load_rgb_image(image_id2, self.transform)

        # since batches are re-ordered for PackedSequence's, the original question order is lost
        # we return `item` so that the order of (v, q, a) triples can be restored if desired
        # without shuffling in the dataloader, these will be in the order that they appear in the q and a json's.
        return v1, v2, q1, q2, a1, a2, item, q_length1, q_length2
